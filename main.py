"""
🍷━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🍷 ЗНАКОМСТВА НА ВИНЧИКЕ — Telegram Dating Bot v5.5
...
class User(Base):
    ...
    rewind_count = Column(Integer, default=0)  # Вернуть анкету
...
class Like(Base):
    ...
    message = Column(Text, nullable=True)  # Сообщение с суперлайком
    created_at = Column(DateTime, default=datetime.utcnow)
...
class TopPick(Base):
    """Топ-подборки для пользователя"""
    __tablename__ = "top_picks"
    id = Column(Integer, primary_key=True)
    user_id = Column(Integer, ForeignKey("users.id"), index=True)
    picked_user_id = Column(Integer, ForeignKey("users.id"))
    category = Column(String(50))  # hot, new, online, compatible
    score = Column(Float, default=0)
    created_at = Column(DateTime, default=datetime.utcnow)


class LikeQueue(Base):
    """Очередь входящих лайков — 'Кто тебя лайкнул'"""
...
class SmartMatch:
    """9-факторная совместимость + icebreaker + ML"""

    @staticmethod
    def calculate(u1: Dict, u2: Dict, learned: Dict = None) -> Dict:
        details = {}

        # 1. БАЗА (25%) — город, возраст, взаимный поиск
        base = 0
        ...
        # 3. ЦЕЛИ (15%)
        g1, g2 = u1.get("relationship_goal"), u2.get("relationship_goal")
        if g1 and g2:
            if g1 == g2: details["goals"] = 100
            elif {g1, g2} & {"not_sure"}: details["goals"] = 60
            elif {g1, g2} == {"serious", "casual"}: details["goals"] = 15
        ...
        # 4. ОБРАЗ ЖИЗНИ (10%)
        life = []
        for field in [("smoking", "smoking"), ("drinking", "drinking"), ("children", "children")]:
            v1, v2 = u1.get(field[0]), u2.get(field[1])
            if v1 and v2:
...
class Learner:
    ...
    async def learn(uid: int):
        async with async_session_maker() as s:
            ...
            top_int = ",".join([k for k, _ in sorted(int_counter.items(), key=lambda x: x[1], reverse=True)[:10]])

            elos = [u.elo_score for u in liked_users if u.elo_score]
            avg_elo = sum(elos) / len(elos) if elos else 1000

            dislikes_r = await s.execute(select(func.count(Dislike.id)).where(Dislike.from_user_id == uid))
            ...
            vals = {
                ...
                "avg_liked_elo": avg_elo, "avg_liked_age": avg_age,
                "like_rate": lr, "total_interactions": len(liked_ids) + total_d,
                "updated_at": datetime.utcnow(),
            }

            ex = await s.execute(select(UserPreferenceLearned).where(UserPreferenceLearned.user_id == uid))
            if ex.scalar_one_or_none():
                await s.execute(update(UserPreferenceLearned).where(UserPreferenceLearned.user_id == uid).values(**vals))
            else:
    ...
    async def get_prefs(uid: int) -> Optional[Dict]:
        async with async_session_maker() as s:
            r = await s.execute(select(UserPreferenceLearned).where(UserPreferenceLearned.user_id == uid))
            p = r.scalar_one_or_none()
            if not p: return None
            return {"preferred_interests": p.preferred_interests, "avg_liked_age": p.avg_liked_age,
                    "avg_liked_elo": p.avg_liked_elo, "total_interactions": p.total_interactions}
...
class Anim:
    ...
    async def animate(msg: Message, frames: List[str], delay=0.4) -> Message:
        ...
        for f in frames[1:]:
            ...
            try: await m.edit_text(f, parse_mode=ParseMode.MARKDOWN)
            except: pass
        return m
...
class DB:
    ...
    @staticmethod
    def is_vip(u): t = u.get("subscription_tier", "free"); return t == "wine_cellar" or (t != "free" and u.get("subscription_expires_at") and u["subscription_expires_at"] > datetime.utcnow())
    @staticmethod
    def is_boosted(u): e = u.get("boost_expires_at"); return e and e > datetime.utcnow()
    @staticmethod
    def is_creator(u): return u.get("telegram_id") in config.CREATOR_IDS
    @staticmethod
    def is_admin(u): return u.get("telegram_id") in config.ADMIN_IDS
    ...
    async def create_user(tg_id, username=None):
        async with async_session_maker() as s:
            u = User(telegram_id=tg_id, username=username, referral_code=str(uuid.uuid4())[:8].upper(), last_limits_reset=datetime.utcnow())
    ...
    async def reset_limits(u):
        ...
        if lr is None or lr.date() < now.date():
            ...
            return await DB.upd(u["telegram_id"],
                daily_likes_remaining=f["likes"], daily_messages_remaining=f["messages"],
                daily_super_likes_remaining=f["super_likes"],
                last_limits_reset=now, last_active_at=now, is_online=True)
        return await DB.upd(u["telegram_id"], last_active_at=now, is_online=True)

    @staticmethod
    async def search(u, limit=1):
        async with async_session_maker() as s:
            liked = {r[0] for r in (await s.execute(select(Like.to_user_id).where(Like.from_user_id == u["id"]))).fetchall()}
            disliked = {r[0] for r in (await s.execute(select(Dislike.to_user_id).where(Dislike.from_user_id == u["id"]))).fetchall()}
            ...
            q = q.order_by(User.boost_expires_at.desc().nullslast(),
                          (User.city == u["city"]).desc(),
                          User.last_active_at.desc()).limit(limit * 10)
            r = await s.execute(q)
        ...
        for c in cands:
            ...
            bonus = 0
            if DB.is_boosted(c): bonus += 15
            if c.get("created_at") and (datetime.utcnow() - c["created_at"]).total_seconds() < 86400: bonus += 10
            if c.get("is_online"): bonus += 8
    ...
    async def add_like(fid, tid, is_super=False, msg_text=None):
        async with async_session_maker() as s:
            ...
            await s.execute(update(User).where(User.id == fid).values(likes_given_count=User.likes_given_count + 1))
            ...
            if is_match:
                ...
                m = Match(user1_id=fid, user2_id=tid, compatibility_score=compat.get("total", 0),
                          compatibility_details=json.dumps(compat.get("details", {})), icebreaker_text=icebreaker)
                s.add(m)
        ...
        asyncio.create_task(EloSystem.on_like(fid, tid))
        asyncio.create_task(Learner.record(fid, tid, "super_like" if is_super else "like"))
        return {"is_match": is_match, "match_id": match_id, "compatibility": compat, "icebreaker": icebreaker}
    ...
    async def add_dislike(fid, tid):
        async with async_session_maker() as s:
            ...
            await s.commit()
        asyncio.create_task(EloSystem.on_dislike(tid))
        asyncio.create_task(Learner.record(fid, tid, "dislike"))

    @staticmethod
    async def get_incoming_likes(uid, limit=10) -> List[Dict]:
        """Получает входящие лайки для 'Кто тебя лайкнул'"""
        async with async_session_maker() as s:
            r = await s.execute(
                select(LikeQueue).where(and_(
                    LikeQueue.target_user_id == uid,
                    LikeQueue.is_revealed == False
                )).order_by(LikeQueue.created_at.desc()).limit(limit)
            )
            out = []
    ...
    async def get_incoming_likes_count(uid) -> int:
        async with async_session_maker() as s:
            r = await s.execute(
                select(func.count(LikeQueue.id)).where(and_(
                    LikeQueue.target_user_id == uid,
                    LikeQueue.is_revealed == False
                ))
            )
    ...
    async def get_match_between(u1, u2):
        async with async_session_maker() as s:
            r = await s.execute(select(Match.id).where(and_(Match.is_active == True,
    ...
    async def send_msg(mid, sid, txt, photo=None, voice=None):
        async with async_session_maker() as s:
            ...
            await s.execute(update(Match).where(Match.id == mid).values(
                last_message_at=datetime.utcnow(), messages_count=Match.messages_count + 1))
            await s.execute(update(User).where(User.id == sid).values(messages_sent_count=User.messages_sent_count + 1))
            await s.commit()
    ...
    async def get_msgs(mid, limit=10):
        async with async_session_maker() as s:
            ...
            return [{"sender_id": m.sender_id, "text": m.text, "photo_id": m.photo_id,
                     "voice_id": m.voice_id, "created_at": m.created_at}
    ...
    async def dec_super(tg_id):
        async with async_session_maker() as s:
            await s.execute(update(User).where(User.telegram_id == tg_id).values(daily_super_likes_remaining=User.daily_super_likes_remaining - 1))
            await s.commit()
    ...
    async def unmatch(uid, mid):
        async with async_session_maker() as s:
            r = await s.execute(select(Match).where(and_(Match.id == mid, Match.is_active == True,
    ...
    async def get_total(): 
        async with async_session_maker() as s:
            return (await s.execute(select(func.count(User.id)).where(User.is_profile_complete == True))).scalar() or 0
    ...
    async def activate_sub(uid, tier, days):
        async with async_session_maker() as s:
            ...
            now = datetime.utcnow()
            exp = None if te == SubscriptionTier.WINE_CELLAR else (
            ...
            if feats.get("who_liked"): vals["who_liked_visible"] = True
    ...
    async def use_promo(uid, code):
        async with async_session_maker() as s:
            ...
            p = r.scalar_one_or_none()
            if not p: return {"error": "❌ Промокод не найден"}
            if p.used_count >= p.max_uses: return {"error": "❌ Исчерпан"}
            used = await s.execute(select(PromoUse).where(and_(PromoUse.promo_id == p.id, PromoUse.user_id == uid)))
            if used.scalar_one_or_none(): return {"error": "❌ Уже использован"}
            s.add(PromoUse(promo_id=p.id, user_id=uid))
            await s.execute(update(PromoCode).where(PromoCode.id == p.id).values(used_count=PromoCode.used_count + 1))
            await s.commit()
    ...
    async def create_payment(uid, yid, amt, desc, ptype, ptier=None, pdur=None, pcnt=None):
        async with async_session_maker() as s:
            p = Payment(user_id=uid, yookassa_payment_id=yid, amount=amt, description=desc, product_type=ptype, product_tier=ptier, product_duration=pdur, product_count=pcnt)
...
class KB:
    ...
    def card(uid, compat=0, can_super=False):
        ...
        rows.append([InlineKeyboardButton(text=f"❤️{c}", callback_data=f"lk:{uid}"),
                     InlineKeyboardButton(text="👎", callback_data=f"dl:{uid}")])
        rows.append([InlineKeyboardButton(text="📊 Подробнее", callback_data=f"cd:{uid}"),
    ...
    def chat(mid, has_icebreaker=False):
        ...
        rows.append([InlineKeyboardButton(text="💕 Мэтчи", callback_data="bm"),
    ...
    def buy_tier(tier):
        prices = {
            ...
            "sommelier": [("799₽/мес", 30, 79900), ("1999₽/3мес -17%", 90, 199900), ("3499₽/6мес -27%", 180, 349900)],
            "wine_cellar": [("4999₽ навсегда 💎", 0, 499900)],
        }
        rows = []
        emoji = TIER_FEATURES.get(tier, {}).get("badge", "💎")
        for label, dur, amt in prices.get(tier, []):
            rows.append([InlineKeyboardButton(text=f"{emoji} {label}", callback_data=f"by:{tier}:{dur}:{amt}")])
    ...
    def boost_menu(has, active):
        ...
        b += [[InlineKeyboardButton(text="🚀 1шт — 99₽", callback_data="by:boost:1:9900")],
    ...
    def interests_picker(selected=None, page=0):
        ...
        pp = 8; start = page * pp; items = INTERESTS_LIST[start:start + pp]
        rows = []
        for i in range(0, len(items), 2):
            row = [InlineKeyboardButton(text=f"{'✅ ' if it in selected else ''}{it}", callback_data=f"int:{it}")
                   for it in items[i:i + 2]]
            rows.append(row)
        nav = []
        if page > 0: nav.append(InlineKeyboardButton(text="⬅️", callback_data=f"int_p:{page - 1}"))
        if start + pp < len(INTERESTS_LIST): nav.append(InlineKeyboardButton(text="➡️", callback_data=f"int_p:{page + 1}"))
    ...
    def report():
        return InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="🔞 18+", callback_data="rr:nsfw"), InlineKeyboardButton(text="😡 Оскорбления", callback_data="rr:harass")],
    ...
    @staticmethod
    def back_admin():
        return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🛡️ Админка", callback_data="adm:main")]])
    ...
    async def __call__(self, handler, event, data):
        ...
        if tg:
            ...
            if u:
                ...
                if u.get("is_banned"):
                    if isinstance(event, Message): await event.answer("🚫 Заблокирован")
                    return
        data["user"] = u
...
async def cmd_start(msg: Message, state: FSMContext, user: Optional[Dict]):
    ...
    if user and user.get("is_profile_complete"):
        ...
        await msg.answer(
            ...
            f"{hint}",
            reply_markup=KB.main(un, lc), parse_mode=ParseMode.MARKDOWN)
    else:
        if not user: await DB.create_user(msg.from_user.id, msg.from_user.username)
...
async def rn(m: Message, s: FSMContext):
    n = m.text.strip()
    if len(n) < 2 or len(n) > 50: await m.answer("⚠️ 2-50 символов"); return
    await s.update_data(name=n)
    await m.answer(f"Привет, *{n}*! 🍷\n🎂 Возраст? _(18-99)_", parse_mode=ParseMode.MARKDOWN)
...
async def ra(m: Message, s: FSMContext):
    try:
        a = int(m.text.strip())
        assert 18 <= a <= 99
    except: await m.answer("⚠️ 18-99"); return
...
async def rg(c: CallbackQuery, s: FSMContext):
    await s.update_data(gender=c.data[2:]); await c.message.edit_text("✅")
    await c.message.answer("🏙️ Город:"); await s.set_state(RegStates.city); await c.answer()

@rt.message(RegStates.city)
async def rc(m: Message, s: FSMContext):
    await s.update_data(city=m.text.strip().title())
    await m.answer("📸 Фото или «Пропустить»:", reply_markup=KB.skip()); await s.set_state(RegStates.photo)

@rt.message(RegStates.photo, F.photo)
async def rp(m: Message, s: FSMContext):
    await s.update_data(photo=m.photo[-1].file_id)
    await m.answer("📝 О себе _(до 500)_ или «Пропустить»:", reply_markup=KB.skip(), parse_mode=ParseMode.MARKDOWN)
    await s.set_state(RegStates.bio)

@rt.callback_query(RegStates.photo, F.data == "skip")
async def rps(c: CallbackQuery, s: FSMContext):
    await s.update_data(photo=None); await c.message.edit_text("📸 Пропущено")
    await c.message.answer("📝 О себе или «Пропустить»:", reply_markup=KB.skip()); await s.set_state(RegStates.bio); await c.answer()

@rt.message(RegStates.bio)
async def rb(m: Message, s: FSMContext):
    await s.update_data(bio=m.text.strip()[:500])
    await m.answer("🔍 Кого ищешь?", reply_markup=KB.looking()); await s.set_state(RegStates.looking_for)

@rt.callback_query(RegStates.bio, F.data == "skip")
async def rbs(c: CallbackQuery, s: FSMContext):
    await s.update_data(bio=""); await c.message.edit_text("🔍 Кого ищешь?", reply_markup=KB.looking())
    await s.set_state(RegStates.looking_for); await c.answer()

@rt.callback_query(RegStates.looking_for, F.data.startswith("l:"))
async def rl(c: CallbackQuery, s: FSMContext):
    await s.update_data(looking_for=c.data[2:]); await c.message.edit_text("✅")
    await c.message.answer("🎯 *Выбери интересы:*", reply_markup=KB.interests_picker(), parse_mode=ParseMode.MARKDOWN)
    await s.update_data(selected_interests=[]); await s.set_state(RegStates.interests); await c.answer()

@rt.callback_query(RegStates.interests, F.data.startswith("int:"))
async def ri(c: CallbackQuery, s: FSMContext):
    i = c.data[4:]; d = await s.get_data(); sel = set(d.get("selected_interests", []))
    if i in sel: sel.discard(i)
    else: sel.add(i)
    await s.update_data(selected_interests=list(sel))
    await c.message.edit_reply_markup(reply_markup=KB.interests_picker(sel, d.get("ip", 0))); await c.answer()

@rt.callback_query(RegStates.interests, F.data.startswith("int_p:"))
async def rip(c: CallbackQuery, s: FSMContext):
    p = int(c.data.split(":")[1]); d = await s.get_data(); await s.update_data(ip=p)
    await c.message.edit_reply_markup(reply_markup=KB.interests_picker(set(d.get("selected_interests", [])), p)); await c.answer()

@rt.callback_query(RegStates.interests, F.data == "int_done")
async def rid(c: CallbackQuery, s: FSMContext):
    d = await s.get_data(); await s.update_data(interests=",".join(d.get("selected_interests", [])))
    await c.message.edit_text(f"🎯 {len(d.get('selected_interests', []))} интересов ✅")
    await c.message.answer("💍 *Цель знакомства?*", reply_markup=KB.goals(), parse_mode=ParseMode.MARKDOWN)
    await s.set_state(RegStates.relationship_goal); await c.answer()

@rt.callback_query(RegStates.relationship_goal, F.data.startswith("goal:"))
async def rgoal(c: CallbackQuery, s: FSMContext):
    d = await s.get_data(); goal = c.data[5:]
    upd = {"name": d["name"], "age": d["age"], "gender": Gender(d["gender"]),
           "city": d["city"], "bio": d.get("bio", ""), "looking_for": LookingFor(d["looking_for"]),
           "interests": d.get("interests", ""), "relationship_goal": goal, "is_profile_complete": True}
    if d.get("photo"): upd["photos"] = d["photo"]; upd["main_photo"] = d["photo"]
    await DB.upd(c.from_user.id, **upd); await s.clear()

    await c.message.edit_text("⏳ Создаём..."); await asyncio.sleep(0.5)
    await c.message.edit_text("🧠 Настраиваем AI..."); await asyncio.sleep(0.5)
    await c.message.edit_text("🎉 *Готово!*", parse_mode=ParseMode.MARKDOWN)

    user = await DB.get_user(c.from_user.id); a = ProfileAnalyzer.analyze(user)
    total = await DB.get_total()
...
async def browse(msg: Message, state: FSMContext, user: Optional[Dict]):
    ...
    if not ps: await m.edit_text("😔 Анкеты закончились. Попробуй позже!"); return
    await m.delete(); await show_card(msg, ps[0], user)
...
async def show_card(msg, p, viewer):
    ...
    except: await msg.answer(txt, reply_markup=kb, parse_mode=ParseMode.MARKDOWN)
...
async def lk(cb: CallbackQuery, user: Optional[Dict]):
    ...
    if not DB.is_vip(user) and user.get("daily_likes_remaining", 0) <= 0:
        await cb.answer("⚠️ Лимит! Подпишись 💎", show_alert=True); return
    tid = int(cb.data[3:]); r = await DB.add_like(user["id"], tid)
    ...
    if r.get("is_match"):
        ...
        # Уведомляем партнёра
        if t:
            try: await cb.bot.send_message(t["telegram_id"],
        ...
        if target and target.get("notify_likes"):
            try:
                if DB.is_vip(target):
                    await cb.bot.send_message(target["telegram_id"],
                        f"❤️ *{user['name']}*, {user['age']} оценил(а) тебя!\n_Листай анкеты — вдруг взаимно!_ 🍷",
                        parse_mode=ParseMode.MARKDOWN)
                else:
                    await cb.bot.send_message(target["telegram_id"],
    ...
    ps = await DB.search(user, 1)
    if ps: await show_card(cb.message, ps[0], user)
    else: await cb.message.answer("😔 Анкеты закончились!")
...
async def sl(cb: CallbackQuery, user: Optional[Dict]):
    ...
    if user.get("daily_super_likes_remaining", 0) <= 0:
        await cb.answer("⚡ Суперлайки кончились! 💎 Премиум = больше!", show_alert=True); return
    tid = int(cb.data[3:]); r = await DB.add_like(user["id"], tid, is_super=True)
    ...
    if r.get("is_match"):
        ...
        target = await DB.get_user_by_id(tid)
        if target:
            try: await cb.bot.send_message(target["telegram_id"],
                f"⚡ *{user['name']}*, {user['age']} отправил(а) тебе суперлайк!*\n_Ты точно понравился!_ 💜",
                parse_mode=ParseMode.MARKDOWN)
            except: pass
...
async def dl(cb: CallbackQuery, user: Optional[Dict]):
    ...
    if ps: await show_card(cb.message, ps[0], user)
        ...
        except: await cb.message.answer("😔 Закончились!")
...
async def compat_details(cb: CallbackQuery, user: Optional[Dict]):
    if not user: return
    tid = int(cb.data[3:]); t = await DB.get_user_by_id(tid)
    if not t: return
...
async def who_liked(msg: Message, user: Optional[Dict]):
    ...
    if count == 0:
        await msg.answer("❤️ Пока никто не лайкнул\n\n🍷 _Листай анкеты — лайки придут!_", parse_mode=ParseMode.MARKDOWN)
        return

    if not DB.is_vip(user):
        # Бесплатные видят только размытый превью
        await msg.answer(
    ...
    # VIP видит кто лайкнул
    likes = await DB.get_incoming_likes(user["id"], 5)
    if not likes:
        await msg.answer("❤️ Нет новых лайков"); return
    ...
    try:
        if fu.get("main_photo"):
            await msg.answer_photo(photo=fu["main_photo"], caption=txt,
                reply_markup=KB.who_liked_card(fu["id"], lk_item["is_super"]),
                parse_mode=ParseMode.MARKDOWN)
        else:
            await msg.answer(txt, reply_markup=KB.who_liked_card(fu["id"], lk_item["is_super"]),
                parse_mode=ParseMode.MARKDOWN)
    except:
        await msg.answer(txt, reply_markup=KB.who_liked_card(fu["id"], lk_item["is_super"]),
            parse_mode=ParseMode.MARKDOWN)
...
async def wl_like(cb: CallbackQuery, user: Optional[Dict]):
    ...
    if r.get("is_match"):
        ...
        try: await cb.message.edit_caption(caption="❤️ *Лайк отправлен!*", parse_mode=ParseMode.MARKDOWN)
        except: pass

@rt.callback_query(F.data.startswith("wl_skip:"))
async def wl_skip(cb: CallbackQuery, user: Optional[Dict]):
    try: await cb.message.edit_caption(caption="👋 Пропущено")
    except: await cb.message.edit_text("👋 Пропущено")
    await cb.answer()
...
async def go_browse(cb: CallbackQuery, state: FSMContext, user: Optional[Dict]):
    ...
    ps = await DB.search(user, 1)
    if ps: await show_card(cb.message, ps[0], user)
    else: await cb.message.answer("😔 Закончились!")
    await cb.answer()
...
async def chats(msg: Message, state: FSMContext, user: Optional[Dict]):
    if not user: return; await state.clear()
    ms = await DB.get_matches(user["id"])
    # Только те, где есть сообщения
    active = [m for m in ms if m.get("messages_count", 0) > 0]
    if active:
        await msg.answer("💬 *Диалоги:*", reply_markup=KB.matches(active), parse_mode=ParseMode.MARKDOWN)
    else:
        await msg.answer("💬 Нет активных чатов\n\n_Напиши своему мэтчу первым!_ 🍷", parse_mode=ParseMode.MARKDOWN)
...
async def open_chat(cb: CallbackQuery, state: FSMContext, user: Optional[Dict]):
    ...
    if not mid: await cb.answer("❌"); return
    ...
    for mg in msgs:
        ...
        if mg.get("photo_id"): txt += f"{sn} 📸 _фото_ {ts}\n"
        elif mg.get("voice_id"): txt += f"{sn} 🎤 _голосовое_ {ts}\n"
        else: txt += f"{sn} {mg['text'] or ''} _{ts}_\n"
    ...
    async with async_session_maker() as s:
        ...
        ib = match.icebreaker_text if match else None

    await state.update_data(cp=pid, mi=mid)
    await state.set_state(ChatStates.chatting)
    await cb.message.edit_text(txt, reply_markup=KB.chat(mid, bool(ib)), parse_mode=ParseMode.MARKDOWN)
    await cb.answer()
...
async def send_txt(msg: Message, state: FSMContext, user: Optional[Dict]):
    ...
    if not mid: await state.clear(); await msg.answer("💬 Закрыт", reply_markup=KB.main()); return
...
async def send_photo(msg: Message, state: FSMContext, user: Optional[Dict]):
    ...
    p = await DB.get_user_by_id(pid)
    if p:
        try: await msg.bot.send_photo(p["telegram_id"], photo_id,
            caption=f"📸 *{user['name']}* отправил(а) фото", parse_mode=ParseMode.MARKDOWN)
        except: pass
    await msg.answer("📸 ✅")
...
async def bm(cb: CallbackQuery, state: FSMContext, user: Optional[Dict]):
    ...
    if not user: return
    ms = await DB.get_matches(user["id"])
    if ms: await cb.message.edit_text(f"💕 *({len(ms)})*", reply_markup=KB.matches(ms), parse_mode=ParseMode.MARKDOWN)
    else: await cb.message.edit_text("😔 Нет мэтчей")

@rt.callback_query(F.data.startswith("um:"))
async def um(cb: CallbackQuery):
    await cb.message.edit_text("💔 *Отвязать?*", reply_markup=KB.confirm_unmatch(int(cb.data[3:])), parse_mode=ParseMode.MARKDOWN)
...
async def daily_match(msg: Message, user: Optional[Dict]):
    ...
    async with async_session_maker() as s:
        ...
        ex = await s.execute(select(DailyMatch).where(and_(
            DailyMatch.user_id == user["id"], DailyMatch.created_at >= today, DailyMatch.is_seen == False)))
    ...
    if not dm:
        ...
        if not ps:
            await msg.answer("🌟 _Пара дня появится завтра!_", parse_mode=ParseMode.MARKDOWN); return
        best = ps[0]
        compat = best.get("_compat", SmartMatch.calculate(user, best))
        ib = SmartMatch.make_icebreaker(user, best, compat)
        async with async_session_maker() as s:
            s.add(DailyMatch(user_id=user["id"], recommended_id=best["id"],
                            compatibility_score=compat["total"], reason=ib))
            await s.commit()
        ...
        ib = dm.reason
        if not rec:
            await msg.answer("🌟 Попробуй позже!"); return
    ...
    try:
        if rec.get("main_photo"):
            await msg.answer_photo(photo=rec["main_photo"], caption=txt,
                reply_markup=KB.daily(rec["id"], total), parse_mode=ParseMode.MARKDOWN)
        else:
            await msg.answer(txt, reply_markup=KB.daily(rec["id"], total), parse_mode=ParseMode.MARKDOWN)
    except:
        await msg.answer(txt, reply_markup=KB.daily(rec["id"], total), parse_mode=ParseMode.MARKDOWN)
...
async def dm_lk(cb: CallbackQuery, user: Optional[Dict]):
    ...
    if r.get("is_match"):
        ...
        try: await cb.message.edit_caption(caption="🌟 *Лайк отправлен!* Ждём ответ 🍷", parse_mode=ParseMode.MARKDOWN)
        except: await cb.message.edit_text("🌟 *Лайк!*", parse_mode=ParseMode.MARKDOWN)
    await cb.answer()
...
async def guests(msg: Message, user: Optional[Dict]):
    ...
    if not gs:
        await msg.answer("👻 Пока нет гостей\n\n💡 _Активируй буст!_", parse_mode=ParseMode.MARKDOWN); return
    txt = "👻 *Кто смотрел:*\n\n"
...
async def profile(msg: Message, user: Optional[Dict]):
    ...
    try:
        if user.get("main_photo"):
            await msg.answer_photo(photo=user["main_photo"], caption=txt, reply_markup=KB.profile(user), parse_mode=ParseMode.MARKDOWN)
        else: await msg.answer(txt, reply_markup=KB.profile(user), parse_mode=ParseMode.MARKDOWN)
    except: await msg.answer(txt, reply_markup=KB.profile(user), parse_mode=ParseMode.MARKDOWN)
...
async def profile_analysis(cb: CallbackQuery, user: Optional[Dict]):
    ...
    txt += f"\n⚖️ Рейтинг: *{user.get('elo_score', 1000)}*\n💕 Привлекательность: *{user.get('attractiveness_score', 50):.0f}%*"
    try: await cb.message.edit_caption(caption=txt, parse_mode=ParseMode.MARKDOWN)
    except: await cb.message.edit_text(txt, parse_mode=ParseMode.MARKDOWN)
    await cb.answer()
...
# Edit handlers (compact)
@rt.callback_query(F.data == "pe")
async def pe(cb: CallbackQuery):
    try: await cb.message.edit_caption(caption="✏️ *Редактировать:*", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✏️ Имя", callback_data="ed:name"), InlineKeyboardButton(text="🎂 Возраст", callback_data="ed:age")],
        [InlineKeyboardButton(text="🏙️ Город", callback_data="ed:city"), InlineKeyboardButton(text="📝 О себе", callback_data="ed:bio")],
        [InlineKeyboardButton(text="⬅️", callback_data="pv")]]), parse_mode=ParseMode.MARKDOWN)
    except: pass
    await cb.answer()
...
async def en(cb: CallbackQuery, state: FSMContext): await cb.message.answer("✏️ Имя:"); await state.set_state(EditStates.edit_name); await cb.answer()
@rt.message(EditStates.edit_name)
async def sn(m: Message, s: FSMContext):
    n = m.text.strip()
    if len(n) < 2: await m.answer("⚠️"); return
    await DB.upd(m.from_user.id, name=n); await s.clear(); await m.answer("✅", reply_markup=KB.main())

@rt.callback_query(F.data == "ed:age")
async def ea(cb: CallbackQuery, s: FSMContext): await cb.message.answer("🎂:"); await s.set_state(EditStates.edit_age); await cb.answer()
@rt.message(EditStates.edit_age)
async def sa(m: Message, s: FSMContext):
    try: a = int(m.text.strip()); assert 18 <= a <= 99
    except: await m.answer("⚠️"); return
    await DB.upd(m.from_user.id, age=a); await s.clear(); await m.answer("✅", reply_markup=KB.main())

@rt.callback_query(F.data == "ed:city")
async def ec(cb: CallbackQuery, s: FSMContext): await cb.message.answer("🏙️:"); await s.set_state(EditStates.edit_city); await cb.answer()
@rt.message(EditStates.edit_city)
async def sc(m: Message, s: FSMContext): await DB.upd(m.from_user.id, city=m.text.strip().title()); await s.clear(); await m.answer("✅", reply_markup=KB.main())

@rt.callback_query(F.data == "ed:bio")
async def eb(cb: CallbackQuery, s: FSMContext): await cb.message.answer("📝:"); await s.set_state(EditStates.edit_bio); await cb.answer()
@rt.message(EditStates.edit_bio)
async def sb(m: Message, s: FSMContext): await DB.upd(m.from_user.id, bio=m.text.strip()[:500]); await s.clear(); await m.answer("✅", reply_markup=KB.main())

@rt.callback_query(F.data == "ed:photo")
async def eph(cb: CallbackQuery, s: FSMContext): await cb.message.answer("📸:"); await s.set_state(EditStates.add_photo); await cb.answer()
@rt.message(EditStates.add_photo, F.photo)
async def sph(m: Message, s: FSMContext, user: Optional[Dict]):
    pid = m.photo[-1].file_id; ph = user.get("photos", ""); ph = (ph + "," + pid) if ph else pid
    await DB.upd(m.from_user.id, photos=ph, main_photo=pid); await s.clear(); await m.answer("📸 ✅", reply_markup=KB.main())
...
async def ei(cb: CallbackQuery, s: FSMContext, user: Optional[Dict]):
    ...
    await s.update_data(selected_interests=list(sel))
    await cb.message.answer("🎯", reply_markup=KB.interests_picker(sel)); await s.set_state(EditStates.edit_interests); await cb.answer()

@rt.callback_query(EditStates.edit_interests, F.data.startswith("int:"))
async def eit(cb: CallbackQuery, s: FSMContext):
    i = cb.data[4:]; d = await s.get_data(); sel = set(d.get("selected_interests", []))
    if i in sel: sel.discard(i)
    else: sel.add(i)
    await s.update_data(selected_interests=list(sel))
    await cb.message.edit_reply_markup(reply_markup=KB.interests_picker(sel)); await cb.answer()

@rt.callback_query(EditStates.edit_interests, F.data.startswith("int_p:"))
async def eip(cb: CallbackQuery, s: FSMContext):
    p = int(cb.data.split(":")[1]); d = await s.get_data()
    await cb.message.edit_reply_markup(reply_markup=KB.interests_picker(set(d.get("selected_interests", [])), p)); await cb.answer()

@rt.callback_query(EditStates.edit_interests, F.data == "int_done")
async def eid(cb: CallbackQuery, s: FSMContext):
    d = await s.get_data(); sel = d.get("selected_interests", [])
    await DB.upd(cb.from_user.id, interests=",".join(sel)); await s.clear()
    await cb.message.edit_text(f"🎯 {len(sel)} ✅"); await cb.answer()

@rt.callback_query(F.data == "ed:goal")
async def eg(cb: CallbackQuery, s: FSMContext):
    await cb.message.answer("💍", reply_markup=KB.goals()); await s.set_state(EditStates.edit_goal); await cb.answer()

@rt.callback_query(EditStates.edit_goal, F.data.startswith("goal:"))
async def sg(cb: CallbackQuery, s: FSMContext):
    await DB.upd(cb.from_user.id, relationship_goal=cb.data[5:]); await s.clear()
    await cb.message.edit_text(f"💍 ✅"); await cb.answer()

@rt.callback_query(F.data == "ed:height")
async def eh(cb: CallbackQuery, s: FSMContext): await cb.message.answer("📏 Рост (см):"); await s.set_state(EditStates.edit_height); await cb.answer()
@rt.message(EditStates.edit_height)
async def sh(m: Message, s: FSMContext):
    try: h = int(m.text.strip()); assert 100 <= h <= 250
    except: await m.answer("⚠️ 100-250"); return
    await DB.upd(m.from_user.id, height=h); await s.clear(); await m.answer("📏 ✅", reply_markup=KB.main())

@rt.callback_query(F.data == "ed:job")
async def ej(cb: CallbackQuery, s: FSMContext): await cb.message.answer("💼:"); await s.set_state(EditStates.edit_job); await cb.answer()
@rt.message(EditStates.edit_job)
async def sj(m: Message, s: FSMContext): await DB.upd(m.from_user.id, job=m.text.strip()[:100]); await s.clear(); await m.answer("💼 ✅", reply_markup=KB.main())
...
async def pr_main(cb: CallbackQuery):
    await cb.message.edit_text("💎 *ПРЕМИУМ*", reply_markup=KB.premium(), parse_mode=ParseMode.MARKDOWN)
    await cb.answer()
...
async def pr_compare(cb: CallbackQuery):
    await cb.message.edit_text(
        ...
        reply_markup=KB.premium(), parse_mode=ParseMode.MARKDOWN)
    await cb.answer()
...
async def pr_tier(cb: CallbackQuery):
    ...
    features_txt = ""
    if feats.get("who_liked"): features_txt += "✅ Видишь кто тебя лайкнул\n"
    if feats.get("invisible"): features_txt += "✅ Режим невидимки\n"
    if feats.get("read_receipts"): features_txt += "✅ Прочтения сообщений\n"
    if feats.get("priority"): features_txt += "✅ Приоритет в выдаче\n"
    if feats.get("top_picks"): features_txt += "✅ Топ-подборки\n"
    if feats.get("super_like_message"): features_txt += "✅ Сообщение с суперлайком\n"
    if feats.get("founder_badge"): features_txt += "✅ Бейдж «Основатель» 👑\n"
    ...
    await cb.message.edit_text(
        ...
        f"{features_txt}",
        reply_markup=KB.buy_tier(tier), parse_mode=ParseMode.MARKDOWN)
    await cb.answer()
...
async def pr_boost(cb: CallbackQuery, user: Optional[Dict]):
    ...
    await cb.message.edit_text(
        ...
        reply_markup=KB.boost_menu(has, act), parse_mode=ParseMode.MARKDOWN)
    await cb.answer()
...
async def boost_act(cb: CallbackQuery, user: Optional[Dict]):
    if not user or user.get("boost_count", 0) <= 0:
        await cb.answer("🚫 Нет бустов!", show_alert=True); return
    ...
    if ok:
        ...
        await cb.message.answer(f"🚀 До {u['boost_expires_at'].strftime('%H:%M')} · Запас: {u['boost_count']}",
...
async def buy(cb: CallbackQuery, user: Optional[Dict]):
    ...
    if not YOOKASSA_AVAILABLE or not config.YOOKASSA_SHOP_ID:
        await cb.answer("💳 Оплата не настроена", show_alert=True); return
    ...
    try:
        ...
        pid = await DB.create_payment(user["id"], p.id, amt, desc,
        ...
        await cb.message.edit_text(f"💳 *{amt / 100:.0f}₽*\n\nОплати и нажми «Проверить» ✅",
            reply_markup=KB.pay(p.confirmation.confirmation_url, pid), parse_mode=ParseMode.MARKDOWN)
...
async def check_pay(cb: CallbackQuery, user: Optional[Dict]):
    ...
    try:
        ...
        if y.status == "succeeded" and p.status != PaymentStatus.SUCCEEDED:
            async with async_session_maker() as s:
                await s.execute(update(Payment).where(Payment.id == pid).values(
            ...
            if p.product_type == "subscription":
                ...
                async with async_session_maker() as s:
                    await s.execute(update(User).where(User.id == p.user_id).values(
                        boost_count=User.boost_count + (p.product_count or 1)))
                    await s.commit()
...
async def promo_input(cb: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(PromoInputState.waiting_code); await cb.answer()
...
async def promo_use(msg: Message, state: FSMContext, user: Optional[Dict]):
    ...
    if "error" in r: await msg.answer(r["error"], reply_markup=KB.main())
    else:
        tn = TIER_NAMES.get(r["tier"], "VIP")
        m = await msg.answer("🎁 Проверяем...")
        await asyncio.sleep(0.5)
        await m.edit_text(f"🎉 *{tn}* на {r['days']} дней! 🍷", parse_mode=ParseMode.MARKDOWN)
...
async def rp_start(cb: CallbackQuery, state: FSMContext):
    ...
    try: await cb.message.edit_caption(caption="⚠️ *Причина:*", reply_markup=KB.report(), parse_mode=ParseMode.MARKDOWN)
    except: await cb.message.edit_text("⚠️ *Причина:*", reply_markup=KB.report(), parse_mode=ParseMode.MARKDOWN)
    await cb.answer()
...
async def rp_save(cb: CallbackQuery, state: FSMContext, user: Optional[Dict]):
    ...
    except: await cb.message.edit_text("✅ Жалоба 🍷")
    ps = await DB.search(user, 1)
    if ps: await show_card(cb.message, ps[0], user)
    await cb.answer()
...
async def mn(cb: CallbackQuery, state: FSMContext, user: Optional[Dict]):
    ...
    await cb.message.answer("🍷", reply_markup=KB.main(un, lc)); await cb.answer()

@rt.callback_query(F.data == "nop")
async def nop(cb: CallbackQuery): await cb.answer()
...
async def admin(msg: Message, state: FSMContext, user: Optional[Dict]):
    ...
    await state.clear()
    await msg.answer(f"🛡️ *Админка*", reply_markup=KB.admin(), parse_mode=ParseMode.MARKDOWN)

@rt.callback_query(F.data == "adm:main")
async def adm_main(cb: CallbackQuery, state: FSMContext, user: Optional[Dict]):
    if not user or not DB.is_admin(user): return
    await state.clear()
    await cb.message.edit_text("🛡️ *Админка*", reply_markup=KB.admin(), parse_mode=ParseMode.MARKDOWN)
    await cb.answer()
...
async def adm_stats(cb: CallbackQuery, user: Optional[Dict]):
    ...
    async with async_session_maker() as s:
        ...
        now = datetime.utcnow(); da = now - timedelta(days=1)
        ...
        rev = (await s.execute(select(func.sum(Payment.amount)).where(Payment.status == PaymentStatus.SUCCEEDED))).scalar() or 0
...
async def bg_learner():
    while True:
        try:
            async with async_session_maker() as s:
                active = await s.execute(select(User.id).where(and_(
                    User.is_active == True, User.likes_given_count > 5)).limit(50))
                for row in active.fetchall():
                    await Learner.learn(row[0]); await asyncio.sleep(0.1)
        except Exception as e: logger.error(f"Learner: {e}")
        await asyncio.sleep(3600)

async def bg_offline():
    """Помечает пользователей оффлайн через 15 мин неактивности"""
    while True:
        try:
            async with async_session_maker() as s:
                threshold = datetime.utcnow() - timedelta(minutes=15)
                await s.execute(update(User).where(and_(
                    User.is_online == True, User.last_active_at < threshold
...
async def main():
    ...
    dp.message.middleware(UserMW()); dp.callback_query.middleware(UserMW())
    dp.include_router(rt)

    asyncio.create_task(bg_learner())
    asyncio.create_task(bg_offline())

    logger.info(f"🍷 {BOT_NAME} v5.5")
    logger.info("🧠 Smart Match: 9-factor + ELO + ML")
    logger.info("💎 Premium: 5 tiers + who liked + invisible + rewinds")
    logger.info("🌟 Daily Match + 💡 Icebreakers + 📸 Photo chat")
    logger.info("❤️ Who Liked + ⚡ Super Likes + 🟢 Online status")
